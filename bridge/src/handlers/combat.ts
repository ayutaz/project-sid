...
export const attack: CommandHandler = async (bot, params) => {
  const { target } = params;
...
export const defend: CommandHandler = async (bot, params) => {
  const { shield } = params;
  if (shield !== false) {
...
export function getCombatHandlers(): Record<string, CommandHandler> {
