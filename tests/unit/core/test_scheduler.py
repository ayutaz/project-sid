...
async def run_n_ticks(scheduler: ModuleScheduler, sas: Any, n: int) -> None:
    """Deterministically execute exactly n ticks (bypasses the async loop)."""
    for _ in range(n):
...
class TestTierExecution:
    """Tests for tier-based execution frequency (deterministic)."""

    async def test_fast_module_every_tick(self) -> None:
        """FAST modules execute on every tick."""
        scheduler = ModuleScheduler()
        fast = DummyFastModule()
        scheduler.register(fast)
        await run_n_ticks(scheduler, None, 5)
    ...
    async def test_mid_module_frequency(self) -> None:
        ...
        scheduler.register(fast)
        scheduler.register(mid)
        await run_n_ticks(scheduler, None, 9)
        # MID fires at ticks 3, 6, 9 = 3 times
    ...
    async def test_slow_module_frequency(self) -> None:
        ...
        scheduler.register(fast)
        scheduler.register(slow)
        await run_n_ticks(scheduler, None, 10)
        # SLOW fires at tick 10 = 1 time
    ...
    async def test_custom_tier_intervals(self) -> None:
        ...
        scheduler = ModuleScheduler(tier_intervals=custom)
        mid = DummyMidModule()
        scheduler.register(mid)
        await run_n_ticks(scheduler, None, 6)
        # MID interval=2: fires at ticks 2, 4, 6 = 3 times
        assert mid.tick_count == 3

    async def test_all_tiers_together(self) -> None:
        """All three tiers coexist correctly over 30 ticks."""
        scheduler = ModuleScheduler()
        fast = DummyFastModule()
        mid = DummyMidModule()
        slow = DummySlowModule()
        ...
        scheduler.register(mid)
        scheduler.register(slow)
        await run_n_ticks(scheduler, None, 30)
        assert fast.tick_count == 30
        assert mid.tick_count == 10  # 3,6,9,...,30
        assert slow.tick_count == 3  # 10,20,30
...
class TestErrorHandling:
    ...
    async def test_error_module_produces_error_result(self) -> None:
        ...
        await run_n_ticks(scheduler, None, 1)
    ...
    async def test_error_does_not_affect_other_modules(self) -> None:
        """Other modules still produce success results even when one errors."""
        scheduler = ModuleScheduler()
        scheduler.register(ErrorModule())
        fast = DummyFastModule()
        scheduler.register(fast)
        await run_n_ticks(scheduler, None, 3)
        fast_result = scheduler.get_latest_result("fast_module")
        error_result = scheduler.get_latest_result("error_module")
        assert fast_result is not None and fast_result.success
        assert error_result is not None and not error_result.success
...
class TestResultCollection:
    ...
    async def test_collect_after_multiple_tick_batches(self) -> None:
        """collect_results accumulates across tick batches until called."""
        scheduler = ModuleScheduler()
        scheduler.register(DummyFastModule())
        await run_n_ticks(scheduler, None, 2)
        await run_n_ticks(scheduler, None, 3)
        results = scheduler.collect_results()
        # 2 + 3 = 5 results total (tick_count continues from previous run_n_ticks)
...
class TestTickInterval:
    ...
    def test_negative_tick_interval_raises(self) -> None:
        scheduler = ModuleScheduler()
        with pytest.raises(ValueError, match="positive"):
            scheduler.tick_interval = -1.0


class TestAsyncTickLoop:
    """Tests that exercise the real async tick loop."""

    async def test_scheduler_runs_and_stops(self) -> None:
        """Verify the real async loop executes ticks."""
        scheduler = ModuleScheduler(tick_interval=0.02)
        fast = DummyFastModule()
        scheduler.register(fast)
        await run_scheduler_for(scheduler, None, 3)
    ...
    async def test_pause_does_not_increment_ticks(self) -> None:
        ...
        # Tick count should not have increased (at most 1 due to race)
